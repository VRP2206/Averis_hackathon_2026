...
| **Move AI model to AWS Bedrock** <Badge type="warning" text="Needs input" /> | AWS bedrock for faster response and muli-model support with scaling as need by user | Really easy migration; no backend code need to change, just the lambda function on AWS |
